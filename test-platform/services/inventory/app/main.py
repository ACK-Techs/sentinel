...
from app.routes.inventory import router as inventory_router  # noqa: E402
from fastapi import FastAPI  # noqa: E402

app = FastAPI(title="inventory")
logger = logging.getLogger("inventory.startup")
manager = get_chaos_manager("inventory", chaos_settings(settings.chaos_token))
...
async def startup_event() -> None:
    if not await verify_schema():
        logger.error(
            "inventory schema missing; run python scripts/seed_db.py before starting services"
        )
        raise SystemExit(1)
