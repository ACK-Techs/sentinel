...
## Operasyon odağı
Bu skill, `llm-local-gemma` başlığında yerel LLM çalıştırmayı "kurulum + çalışma zamanı + arıza kurtarma" üçlüsüyle ele alır. Amaç yalnızca modeli ayağa kaldırmak değil; Sentinel CLI profillerinde (`local`, `openai-compatible`, `anthropic`) davranış farklarını yönetilebilir hale getirmektir.
