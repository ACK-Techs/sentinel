...
## Tasarım niyeti
`llm-prompt-classification` için hedef; promptu "metin yazımı" olarak değil, ajan davranış sözleşmesi olarak ele almaktır. Prompt; araç çağrısı sınırlarını, cevap formatını ve başarısızlık durumunda geri dönüş stratejisini açıkça taşımalıdır.
...
- Siniflandirma promptunda label setini kapali liste yap ve "other" kurali tanimla. Dengesiz siniflarda macro-F1 odakli degerlendirme notu ekle.
