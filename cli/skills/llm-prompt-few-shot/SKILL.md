...
## Tasarım niyeti
`llm-prompt-few-shot` için hedef; promptu "metin yazımı" olarak değil, ajan davranış sözleşmesi olarak ele almaktır. Prompt; araç çağrısı sınırlarını, cevap formatını ve başarısızlık durumunda geri dönüş stratejisini açıkça taşımalıdır.
