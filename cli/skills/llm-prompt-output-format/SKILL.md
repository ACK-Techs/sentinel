...
## Tasarım niyeti
`llm-prompt-output-format` için hedef; promptu "metin yazımı" olarak değil, ajan davranış sözleşmesi olarak ele almaktır. Prompt; araç çağrısı sınırlarını, cevap formatını ve başarısızlık durumunda geri dönüş stratejisini açıkça taşımalıdır.
...
- Cikti formatinda birincil parser hedefini tek yapida tut (ornegin yalniz JSON object). "Ek aciklama" ihtiyaci icin ayri alan tanimla, serbest metin birakma.
