...
## Tasarım niyeti
`llm-prompt-chain-of-thought` için hedef; promptu "metin yazımı" olarak değil, ajan davranış sözleşmesi olarak ele almaktır. Prompt; araç çağrısı sınırlarını, cevap formatını ve başarısızlık durumunda geri dönüş stratejisini açıkça taşımalıdır.
...
- Uretim modunda gizli dusunce isteme yerine adim basliklari iste. CoT sinyalini testte acik, son kullanici cevabinda ise ozetlenmis akil yurutme seviyesinde tut.
