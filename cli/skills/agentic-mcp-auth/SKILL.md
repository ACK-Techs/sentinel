...
## MCP bağlamı
`agentic-mcp-auth` için odak, Model Context Protocol bileşenlerini Sentinel agent döngüsüne öngörülebilir şekilde bağlamaktır. Burada hata çoğunlukla koddan değil; şema, transport veya auth sözleşmesindeki küçük uyumsuzluklardan çıkar.
...
- MCP auth icin token scope, suresi ve yenileme akislarini acik belgeleyin. Server loglarinda auth header degerini asla yazma.
